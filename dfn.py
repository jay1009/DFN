...
class SmoothNet(nn.Module):
    ...
    def forward(self, x):
        ...
        h3 = self.rrb_3(h3)
        h4 = self.rrb_4(h4)
        h4_up = F.upsample(glob_pool, size=(self.H // 32, self.W // 32), mode='bilinear',align_corners=True) # 1/32
        #h4_up = Upsample(glob_pool, (self.H // 32, self.W // 32)) # 1/32
        #h4_up = F.interpolate(glob_pool, size=(self.H // 32, self.W // 32), mode='bilinear', align_corners=False)
        h4_up = self.bottleneck_1(h4_up)

        h4 = self.rrb_last(self.cab(h4, h4_up)) # 1/32
        #h3_up = Upsample(h4, size=(self.H // 16, self.W // 16), mode='bilinear') # 1/32
        h3_up = F.upsample(h4, size=(self.H // 16, self.W // 16), mode='bilinear',align_corners=True)
        #h3_up = F.interpolate(h4, size=(self.H // 16, self.W // 16), mode='bilinear', align_corners=False)
        h3 = self.rrb_last(self.cab(h3, h3_up)) # 1/16
        #h2_up = Upsample(h3, size=(self.H // 8, self.W // 8), mode='bilinear') # 1/32
        h2_up = F.upsample(h3, size=(self.H // 8, self.W // 8), mode='bilinear',align_corners=True)
        #h2_up = F.interpolate(h3, size=(self.H // 8, self.W // 8), mode='bilinear', align_corners=False)
        h2 = self.rrb_last(self.cab(h2, h2_up)) # 1/8
        #h1_up = Upsample(h2, size=(self.H // 4, self.W // 4), mode='bilinear') # 1/32
        h1_up = F.upsample(h2, size=(self.H // 4, self.W // 4), mode='bilinear',align_corners=True)
        #h1_up = F.interpolate(h2, size=(self.H // 4, self.W // 4), mode='bilinear', align_corners=False)
        h1 = self.rrb_last(self.cab(h1, h1_up)) # 1/4

        # 4x up
        #out = Upsample(h1, size=(self.H, self.W), mode='bilinear') # 1/32
        out = F.upsample(h1, size=(self.H, self.W), mode='bilinear',align_corners=True)
        #out = F.interpolate(h1, size=(self.H, self.W), mode='bilinear', align_corners=False)
